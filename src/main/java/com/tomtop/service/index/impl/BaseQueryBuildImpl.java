...
public abstract class BaseQueryBuildImpl implements IBaseQueryBuild{
	...
	/**搜索的字段**/
	public  String []fields = {"listingId","sku","brand","mutil.title","mutil.desc","mutil.productTypes.productTypeName","mutil.keywords","mutil.items.value","spu"};
	...
	public QueryBuilder multiMatchQuery(String key,String ...fields){
		return QueryBuilders.multiMatchQuery(key,fields);  
	}
	...
	public QueryBuilder buildQueryBuilder(String key,List<Filter> list){
		...
		String fields[] ={"listingId","sku","brand","mutil.title","mutil.desc","mutil.productTypes.productTypeName","mutil.keywords","mutil.items.value","spu"};
	...
	private SearchRequestBuilder autoOrder(SearchRequestBuilder requestBuilder,List<OrderEntity> orders,int productTypeId){
		...
		for (OrderEntity orderEntity : orders) {
			...
			if(productTypeId>0 && StringUtils.equals(orderEntity.getPropetyName(), "mutil.productTypes.sort")){
				...
				//TODO 根据类目ID获取它的类目级别，然后在进行过滤
				
				TermQueryBuilder termFilter = QueryBuilders.termQuery("mutil.productTypes.productTypeId", productTypeId);
				BoolQueryBuilder bool= QueryBuilders.boolQuery().must(termFilter);
				
				NestedQueryBuilder nq = QueryBuilders.nestedQuery("mutil.productTypes",bool);
				
				FieldSortBuilder fieldSort = SortBuilders.fieldSort("mutil.productTypes.sort").setNestedFilter(nq).order(SortOrder.ASC);
